...
{
    _captionLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, _scrollView.frame.size.width, 30)];
            ...
            if([[aImageUrls objectAtIndex:i] isKindOfClass:[UIImage class]]) {
                ...
                [self.imageSource imageWithUrl:imageUrl
                                    completion:^(UIImage *image, NSError *error)
                 {
                     ...
                     [imageView setContentMode:[self->_dataSource contentModeForImage:i inPager:self]];

                     // Stop and Remove Activity Indicator
                     UIActivityIndicatorView *indicatorView = (UIActivityIndicatorView *)[self->_activityIndicators objectForKey:[NSString stringWithFormat:@"%d", i]];
                     if (indicatorView) {
                         [indicatorView stopAnimating];
                         [self->_activityIndicators removeObjectForKey:[NSString stringWithFormat:@"%d", i]];
                     }
                 }];
            }
